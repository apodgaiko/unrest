...
Gated by UNREST_SMOKE_REAL_ACP so the default suite stays hermetic.
"""
...
from unrest_harness.models import Task, TaskList, TerminalReviewHandoff
...
_CLAUDE_ACP = (
    os.environ.get("UNREST_SMOKE_CLAUDE_ACP_CMD")
    or os.environ.get("UNREST_SMOKE_ACP_CMD")
    or "claude-agent-acp"
)
_CODEX_ACP = (
    os.environ.get("UNREST_SMOKE_CODEX_ACP_CMD")
    or os.environ.get("UNREST_SMOKE_ACP_CMD")
    or "codex-acp"
)
_MID = "mission-001"
_N = int(os.environ.get("UNREST_SMOKE_N", "2") or "2")


def _have_command(command: str) -> bool:
    return bool(os.environ.get("UNREST_SMOKE_ACP_CMD")) or bool(shutil.which(command))
...
def _write_skill(store: ProjectStore, pid: str) -> None:
    d = store.unrest_dir(pid) / "skills" / "noop-file-worker"
...
@pytest.mark.skipif(
    ...
    reason=(
        "Set UNREST_SMOKE_REAL_ACP=claude and put claude-agent-acp on PATH "
        "(or set UNREST_SMOKE_CLAUDE_ACP_CMD) to enable this smoke test."
    ),
)
...
@pytest.mark.skipif(
    ...
    reason=(
        "Set UNREST_SMOKE_REAL_ACP=codex and put codex-acp on PATH "
        "(or set UNREST_SMOKE_CODEX_ACP_CMD) to enable this smoke test."
    ),
)
