...
def test_unsupported_profile_has_stable_provider_role_version_error(profile: str) -> None:
    ...
    with pytest.raises(
        CapabilityPolicyError,
        match=rf"provider=claude role=worker version=1 capability=profile:{profile}",
    ):
        validate_provider_support(
            PROVIDERS["claude"],
            role="worker",
...
def test_unknown_runtime_provider_has_stable_role_version_error(
    ...
    with pytest.raises(
        CapabilityPolicyError,
        match=(
            "provider=unknown-provider role=orchestrator "
            "version=1 capability=provider"
        ),
    ):
...
def test_cli_rejects_unsupported_hermes_orchestrator_before_writes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace = tmp_path / "workspace"
    ...
    assert (
        "provider=hermes role=orchestrator version=1 capability=role"
        in result.output
    )
