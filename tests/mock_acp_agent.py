...
Handles:
  ...
  - session/prompt: writes a synthesized handoff payload directly to
    `UNREST_HANDOFF_PATH` (mimicking what the worker MCP server would do
    in production), then returns stop reason `end_turn`.

Environment variables read from os.environ:
  UNREST_HANDOFF_PATH  — where to write the synthesized handoff file
  UNREST_NODE_ID       — node id to embed in handoff
  UNREST_NODE_TYPE     — work | validate (controls the handoff shape)
  UNREST_VALIDATION_PASSED — '1' to mark items passed=true (validate only)
...
def _write_handoff() -> None:
    path = os.environ.get("UNREST_HANDOFF_PATH")
    if not path:
        return
