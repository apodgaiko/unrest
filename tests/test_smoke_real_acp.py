...
LLM-credit consumption:
    ...
    UNREST_SMOKE_REAL_ACP=claude pytest tests/test_smoke_real_acp.py -s
    UNREST_SMOKE_REAL_ACP=codex  pytest tests/test_smoke_real_acp.py -s
    UNREST_SMOKE_REAL_ACP=both   pytest tests/test_smoke_real_acp.py -s

Per-test timeouts default to 300 seconds (covers a cold-cache LLM run);
override via UNREST_SMOKE_TIMEOUT_SECONDS=N.
...
with that content. We verify:
  ...
  2. The attempt JSON handoffs land in the runtime cursor tree
     (`<bucket>/.unrest-runtime/missions/<mid>/attempts/<ts>__<node>.json`)
     and the agent-readable MD mirrors in the durable `.unrest/` record.
...
_FLAG = os.environ.get("UNREST_SMOKE_REAL_ACP", "").strip().lower()
_TIMEOUT_S = int(os.environ.get("UNREST_SMOKE_TIMEOUT_SECONDS", "300"))
...
class _CleanTerminalReviewer:
    ...
    def review(self, project_id: str, mission_id: str, spawn_ts: str):
        from unrest_harness.models import TerminalReviewHandoff
    ...
    [
        pytest.param(
            ...
            marks=pytest.mark.skipif(
                ...
                reason=(
                    "Set UNREST_SMOKE_REAL_ACP=claude (or both) and install "
        ...
        pytest.param(
            ...
            marks=pytest.mark.skipif(
                ...
                reason=(
                    "Set UNREST_SMOKE_REAL_ACP=codex (or both) and install "
...
def test_smoke_hello_mission(
    ...
    # 5) Attempt JSON handoffs in the runtime cursor tree (.unrest-runtime/)
    ...
    # ...and agent-readable MD mirrors in the durable .unrest/ record.
    ...
    from unrest_harness.models import ValidateHandoff
    ...
    # 7) Closeout written
    closeout = workspace / ".unrest" / "missions" / "mission-001" / "closeout.md"
...
@pytest.mark.skipif(
    ...
    reason="Set UNREST_SMOKE_REAL_ACP=claude (or both) + install claude-agent-acp.",
)
...
@pytest.mark.skipif(
    ...
    reason="Set UNREST_SMOKE_REAL_ACP=codex (or both) + install codex-acp.",
)
...
def _run_with_real_terminal_reviewer(
    ...
    review_dir = workspace / ".unrest" / "missions" / "mission-001" / "terminal-reviews"
