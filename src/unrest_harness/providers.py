...
class ProviderSelection:
    ...
    def env(self) -> dict[str, str]:
        env: dict[str, str] = {
            ...
            "UNREST_WORKER_PROVIDER": self.worker.name,
        }
        if self.resolved_worker_acp_command:
            env["UNREST_WORKER_ACP_COMMAND"] = self.resolved_worker_acp_command
        if self.resolved_validation_worker.name != self.worker.name:
            env["UNREST_VALIDATOR_PROVIDER"] = self.resolved_validation_worker.name
