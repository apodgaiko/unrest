...
reverts. Rebuild the wheel from the restored tree and run the frozen-candidate
release checkpoint (`env -u CODEX_PATH uv run pytest -q` on Python 3.13),
focused archive check, and unrelated-cwd installed-wheel lifecycle before
republishing or reinstalling. Do not delete project records: this change
introduces no storage
...
check-repository`, the single Python 3.13 full suite, wheel/sdist archive
checks, and installed-candidate smoke surfaces. If credential exposure
triggered rollback, preserve the
