...
- `SEC-CAPABILITY-001`: one versioned role policy resolves before child
  startup and bounds ACP callbacks, subprocess environments, and approvals.
...
## Capability resolution and ACP callbacks

The canonical [role capability policy](../architecture/capability-policy.md)
is loaded and provider support is checked before either the role MCP server or
ACP agent starts. Worker, validator, and reviewer projections resolve their own
canonical roots and environment declarations; no role inherits the worker
projection.

ACP filesystem callbacks enforce canonical read/write roots. Terminal creation
enforces process availability, canonical cwd, environment names, and
shell-free structured execution. ACP permission requests select only the
policy-declared one-shot outcome. Unsupported profiles, versions, roles, or
network modes raise `CAP-POLICY-001` before dispatch and never enter a mock
fallback.
...
observation. Current behavior is governed by
[`ARCH-CAPABILITY-001`](../architecture/capability-policy.md).
