...
- Capability/provider/version/profile failures occur before child startup with
  `CAP-POLICY-001`. Orchestrator startup has no diagnostic mock fallback.
...
and remains historical characterization. Current enforcement is specified by
the [role capability policy](../architecture/capability-policy.md).
