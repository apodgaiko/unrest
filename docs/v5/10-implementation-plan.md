...
| Validate | typed validation handoff and gate aggregation | contract cursor, attempts, gate attention | `test_coordinator.py`, `test_baseline.py` |
...
The baseline's overlapping mutable-worker batching and implicit unrestricted
provider modes are explicitly non-normative. Their characterization fixtures
remain evidence for the Batch 0 repairs, not completion criteria for this plan.
